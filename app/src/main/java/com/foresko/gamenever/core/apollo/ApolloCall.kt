...
suspend fun <D : Operation.Data> ApolloCall<D>.executeCatching(): Either<TechnicalError, ApolloOperationResult<D>> {
    return try {
        ApolloOperationResult.valueOf(execute()).right()
    } catch (throwable: Throwable) {
        throwable.toTechnicalError().left()
    }
}
